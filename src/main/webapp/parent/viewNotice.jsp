...
<head>
    ...
    <style>
        body { font-family:Arial,sans-serif; background:linear-gradient(135deg,#48bb78,#38a169); margin:0; padding:20px; }
        .container { max-width:900px; margin:0 auto; background:#fff; padding:30px; border-radius:15px;
                     box-shadow:0 10px 30px rgba(0,0,0,0.3); }
        h1 { text-align:center; color:#333; margin-bottom:10px; }
        .subtitle { text-align:center; color:#666; margin-bottom:20px; }
        .back-btn { padding:8px 18px; background:#95a5a6; color:#fff; text-decoration:none; border-radius:5px;
                    font-weight:bold; display:inline-block; margin-bottom:15px; }
        .notice-list { margin-top:10px; }
        .notice-card { border:1px solid #e2e8f0; border-radius:10px; padding:18px; margin-bottom:15px;
                       background:#f9fafb; }
        .notice-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; }
        .notice-title { font-weight:bold; font-size:16px; color:#2d3748; }
        .badge { padding:4px 10px; border-radius:999px; font-size:11px; font-weight:bold; }
        .badge-category { background:#667eea; color:#fff; }
        .badge-audience { background:#e2e8f0; color:#2d3748; margin-left:6px; }
        .notice-meta { font-size:12px; color:#718096; margin-bottom:8px; }
        .notice-message { font-size:14px; color:#4a5568; white-space:pre-wrap; }
        .no-data { text-align:center; padding:40px 10px; color:#999; }
    </style>
</head>
<body>
<div class="container">
    ...
    <h1>📢 Notice Board</h1>
    <p class="subtitle">Dear <strong><%= parentName %></strong>, please review the latest notices from school.</p>
    ...
    <div class="notice-list">
        <%
            try {
                ...
                // ✅ Notices for Parent or All (NEW values saved by AddNoticeServlet)
                ArrayList<Document> list = notices
                        .find(new Document("audience", new Document("$in",
                                java.util.Arrays.asList("ALL", "PARENT"))))
